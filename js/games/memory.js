...
const GameMemory = (() => {
  ...
  function getConfig(level) {
    ...
    if (level === 10) return { pairs: 10, cols: 'cols-4' };
  ...
  function render(cfg) {
    ...
    el.innerHTML = `
      ...
      <div class="level-btns" style="margin-bottom:16px">
        <button class="level-btn ${currentLevel===6?'active':''}" onclick="GameMemory.changeLevel(6)">🌱 Fácil (6)</button>
        <button class="level-btn ${currentLevel===8?'active':''}" onclick="GameMemory.changeLevel(8)">⭐ Normal (8)</button>
        <button class="level-btn ${currentLevel===10?'active':''}" onclick="GameMemory.changeLevel(10)">🔥 Difícil (10)</button>
      </div>
