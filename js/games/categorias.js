const GameCategorias = (() => {
  ...
  function getQueue() {
    if (!queue) queue = Shared.createQueue(DATA.categorias.length);
    return queue;
  }

  function getPool() {
    const all = DATA.categorias;
  ...
  function render() {
    ...
    answered = false;
    container.innerHTML =
      (!sessionCallback ? Shared.levelSelector(level, 'GameCategorias') : '') +
      ...
      opts.map(function(opt) {
        return '<button class="option-btn" onclick="GameCategorias.answer(this, \'' + opt.replace(/'/g,"\\'") + '\')">' + opt + '</button>';
      }).join('') +
      '</div>';
  }
  ...
  function onFinish() {
    Progress.recordGame('categorias', correct, questions.length);
    if (sessionCallback) { sessionCallback(correct >= Math.ceil(questions.length / 2)); return; }
    document.getElementById('game-container').innerHTML =
      Shared.resultScreen(correct, questions.length, 'GameCategorias.loadQuestions()', 'App.showScreen(\'home\')');
  }
